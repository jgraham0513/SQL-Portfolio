...
/*
Created a table with certain information on SuperBowls
*/
...
/*Select average, max, and min scoring totals for winning teams and losing teams in the Superbowl era*/
...
/*Select all teams that have won more than 3 superbowls*/

SELECT Winner, COUNT(*) AS SuperBowls_Won FROM superbowls
    GROUP BY Winner
    HAVING SuperBowls_Won >=3;

/*This is a CASE statement to determine and classify the outcome of all SuperBowl games. Classifying them as either a close game, a decent game, or a blowout, all based on the point differential between the two teams playing*/

SELECT SB, Winner, Winner_Pts, Loser, Loser_Pts, CASE
    WHEN Winner_Pts - Loser_Pts <= 7 THEN "Close Game"
    WHEN Winner_Pts - Loser_Pts >7 AND Winner_Pts - Loser_Pts < 14 THEN "Decent Game"
    ELSE "Blowout"
    END "End Result"
    FROM superbowls
    ORDER BY "End Result";

/*Select all teams that have won a SuperBowls played in Tampa or Miami while scoring more than 24 points*/

SELECT SB, Winner, Winner_Pts, City FROM superbowls WHERE City = "Tampa" OR City = "Miami" AND Winner_Pts >= 24;

/*Select SuperBowl winner, the game's MVP, and the city, when the game was played in the state of Texas*/

SELECT SB, Winner, MVP, City FROM superbowls WHERE State = "Texas";
